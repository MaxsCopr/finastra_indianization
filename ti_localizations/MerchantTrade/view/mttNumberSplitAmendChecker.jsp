...
<script type="text/javascript">
	$(document).ready(function() {
		$("#search").click(function(){
			var masterRef = $.trim($('#mttSplitCheckerMasRef').val());
			if(masterRef.length == 0){
				alert("Please Enter Master Ref.");
				return false;
			}
			var eventRef = $.trim($('#mttSplitCheckerEveRef').val());
			if(eventRef.length == 0){
				alert("Please Enter Event Ref.");
				return false;
			}
			 $("#form1").attr("action","mttfetchSplitCheckerDataForApproval");
			 $("#form1").submit();
		});  	 
		$("#reset").click(function(){
			 $("#form1").attr("action","mttNumberSplitCheckerProcess");
			 $("#form1").submit();
		}); 
		$("#ok").click(function() {
			$("#splitCheck").val("Approve");
			var remark = $.trim($('#splitCheckerRemarks').val());
			if(remark.length == 0){
				alert("Remarks must be filled out");
				return false;
			}
			$('#form1').attr('action','approveSplitCheckerData');
			$('#form1').submit();
 
		});
		$("#reject").click(function() {
			$("#splitCheck").val("Reject");
			var remark = $.trim($('#splitCheckerRemarks').val());
			if(remark.length == 0){
				alert("Remarks must be filled out");
				return false;
			}		
			$('#form1').attr('action','rejectSplitCheckerData');
			$('#form1').submit();
...
<mtt:textfield id="mttSplitCheckerMasRef" name="mttVO.mttSplitCheckerMasRef"
...
<div class="col-md-3 input-group input-group-md">
<mtt:textfield id="mttSplitCheckerEveRef" name="mttVO.mttSplitCheckerEveRef"
													cssClass="form-control text_box"  />
</div>
</div>
</div>
...
<div class="page_collapsible" id="body-section1">
<span></span>
<h5 style="font-weight: bold; font-size: 13px;">&nbsp;MTT Split Amend Pending List</h5>
</div>
<div class="form-group">
...
<th style="text-align: left; width:300px; padding:4px 5px;"><label >MTT Amount</label></th>
...
<mtt:property value="mttSplitListMTTNumber"/>														
<div class="col-md-8 input-group input-group-md"></div>
</div>
</td>
<td style="padding:4px 5px;">
<div class="form-group" align="left">
<mtt:property value="mttSplitListMTTAmount"/>
<div class="col-md-8 input-group input-group-md"></div>
</div>
</td>
<td style="padding:4px 5px;">
<div class="form-group" align="left">
<mtt:property value="mttSplitListMakerUserId"/>														
<div class="col-md-8 input-group input-group-md"></div>
</div>
</td>
<td style="padding:4px 5px;">
<div class="form-group" align="left">
<mtt:property value="mttSplitListMakertmstmp"/>														
<div class="col-md-8 input-group input-group-md"></div>
</div>
</td>
</tr>
...
<mtt:textarea name="splitCheckerRemarks" id="splitCheckerRemarks" cols="15" rows="5"></mtt:textarea>
<div class="col-md-8 input-group input-group-md"></div>
</div>
</div>	
</div>
<input type="hidden" id="splitCheck" name="splitCheck"  />
