...
@WebServlet({"/SwiftOutProcess"})
public class SwiftOutProcess
  extends HttpServlet
{
    ...
    throws ServletException, IOException
  {
    ...
    String filePath = "/ftrade/FTI_Repositories/Localisation_logs/Localization/ThemeBridge/logs/Webswiftout";
    try
    {
      String swiftMessage = request.getParameter("swiftOutMsg");
      System.out.println("TI Swift Message ---> " + swiftMessage);
      ...
      String outputQName = (String)CommonMethods.TBProperties.get("Test_OutMQName");
      queueResult = writeMQMessage(swiftMessage, port, hostname, channel, qManager, outputQName);
      if (!queueResult.equalsIgnoreCase("failed"))
      {
        System.out.println("Queue Result for SWIFT OUT : " + queueResult);
  ...
  public static String writeMQMessage(String inputMessage, int port, String hostname, String channel, String qManager, String outputQName)
  {
    ...
    try
    {
      ...
      try
      {
        System.out.println("Swift out Queue manager connection check process");
