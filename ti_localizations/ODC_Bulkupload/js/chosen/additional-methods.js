...
jQuery.validator
		.addMethod(
				"accept",
				function(value, element, param) {
					// Split mime on commas in case we have multiple types we
					// can accept
					var typeParam = typeof param === "string" ? param.replace(
							/\s/g, '').replace(/,/g, '|') : "image/*", optionalValue = this
					...
					if ($(element).attr("type") === "file") {
						...
						// Check if the element has a FileList before checking
						// each file
						if (element.files && element.files.length) {
							for (i = 0; i < element.files.length; i++) {
								file = element.files[i];
 
								// Grab the mimtype from the loaded file, verify
								// it matches
					...
					// Either return true because we've validated each file, or
					// because the
					// browser does not support element.files and the FileList
					// feature
					return true;
